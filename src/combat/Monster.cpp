...
void Monster::applyEndOfRoundPowers(BattleContext &bc) {
  ...
  if (hasStatus<MS::RITUAL>()) { // sort of a hack, RitualPower contains a
                                 // skipFirst boolean
...
void Monster::construct(BattleContext &bc, MonsterId monsterId,
                        int monsterIdx) {
  this->id = monsterId;
  this->idx = monsterIdx;
...
bool Monster::isAlive() const { return curHp > 0; }

bool Monster::isTargetable() const { return !isDeadOrEscaped(); }

bool Monster::isDying() const { return curHp <= 0; }

bool Monster::isEscaping() const { return isEscapingB; }

bool Monster::isDeadOrEscaped() const {
  return isDying() || isHalfDead() || isEscaping();
}

bool Monster::isHalfDead() const { return halfDead; }

bool Monster::doesEscapeNext() const { return escapeNext; }

bool Monster::isAttacking() const { return isMoveAttack(moveHistory[0]); }
...
void Monster::addBlock(int amount) { block += amount; }

void Monster::die(BattleContext &bc) {
  --bc.monsters.monstersAlive;

  if (id == MonsterId::AWAKENED_ONE &&
      !miscInfo) { // is awakened one stage 1 // todo change to status
    halfDead = true;
    removeDebuffs();
    ...
    //            bc.cleanCardQueue(); // todo should this really return like
    //            this?
  ...
  if (bc.player.hasRelic<RelicId::THE_SPECIMEN>()) {
    bc.addToBot(
        Actions::SetState(InputState::SELECT_ENEMY_THE_SPECIMEN_APPLY_POISON));
  }
}
...
void Monster::attackedUnblockedHelper(
    BattleContext &bc,
    int damage) { // todo, is this supposed to be only damage > 0 ????
  if (bc.player.hasRelic<RelicId::THE_BOOT>() && damage > 0 && damage < 5) {
    damage = 5;
  }

  if (bc.player.hasStatus<PS::ENVENOM>()) {
    bc.addToTop(Actions::DebuffEnemy<MS::POISON>(
        this->idx, bc.player.getStatus<PS::ENVENOM>()));
  }
...
// this is calculated at the player when the damage occurs in game, consider
// testing whether there are any scenarios when it can't be done before
int Monster::calculateDamageToPlayer(const BattleContext &bc,
  ...
  if (p.hasStatus<PS::SURROUNDED>()) { // todo this is probably wrong
    const bool facingSelf =
        p.lastTargetedMonster == idx ||
...
bool Monster::firstTurn() const { return moveHistory[0] == MMID::INVALID; }

bool Monster::lastMove(MMID moveId) const { return moveHistory[0] == moveId; }
...
std::ostream &printIfHaveStatus(std::ostream &os, const Monster &m,
                                MonsterStatus s, bool &havePrint) {
...
std::ostream &operator<<(std::ostream &os, const Monster &m) {
  os << "{";
  os << m.idx << " " << sts::monsterIdStrings[(int)m.id] << " hp:(" << m.curHp
     << "/" << m.maxHp << ")"
     << " block:(" << m.block << ") statusEffects:{";

  bool havePrint = false;
  printIfHaveStatus(os, m, MS::STRENGTH, havePrint);
  for (int i = static_cast<int>(MS::ARTIFACT);
       i <= static_cast<int>(MS::STASIS); ++i) {
  ...
  os << " halfDead: " << m.halfDead << ", moveHistory: { "
     << monsterMoveStrings[(int)m.moveHistory[0]] << ", "
     << monsterMoveStrings[(int)m.moveHistory[1]] << "}"
     << " miscInfo: " << m.miscInfo << " uniquePower0: " << m.uniquePower0
